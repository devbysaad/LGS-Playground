...
def main() -> None:
    ...
    for y in range(OUT_H):
        for x in range(OUT_W):
            # denser grass dither so the lawn reads as pixel texture, not flat fill
            n = (x * 17 + y * 31) & 255
            if n % 11 == 0:
                gid = GRASS_C
            elif n % 5 == 0:
    ...
    for spec in WINGS:
        ...
        for floor in ("ground", "first"):
            ...
            add_prop("locker", wing.stair_tile[0] - 4, wing.cy0 + 1)
            add_prop("locker", wing.stair_tile[0] + 4, wing.cy0 + 1)
            # Corridor runners every ~16 tiles along the main hall
            for rug_x in range(wing.mx0 + 8, wing.mx1 - 6, 16):
                add_prop("rug", rug_x, wing.cmid)
            ...
            if is_ground:
                ...
                add_prop("fountain", court_x0 + COURT_W // 2, wing.sy0 + SOUTH_H // 2)
                add_prop("tree", court_x0 + 3, wing.sy0 + 2)
                add_prop("tree", court_x1 - 3, wing.sy0 + 2)
                add_prop("tree-sm", court_x0 + 4, wing.sy1 - 3)
                add_prop("tree-sm", court_x1 - 4, wing.sy1 - 3)
                add_prop("bush", court_x0 + 2, wing.sy0 + SOUTH_H // 2)
                add_prop("bush", court_x1 - 2, wing.sy0 + SOUTH_H // 2)
                add_prop("flower", court_x0 + 5, wing.sy0 + 5)
                add_prop("flower", court_x1 - 5, wing.sy0 + 5)
                add_prop("bench", court_x0 + COURT_W // 2 - 3, wing.sy1 - 2)
                add_prop("bench", court_x0 + COURT_W // 2 + 3, wing.sy1 - 2)
                add_prop("planter", court_x0 + COURT_W // 2, wing.sy0 + 2)
                ...
                for tx in range(lab_x0 + 3, court_x1 - 2, 8):
                    add_prop("planter", tx, wing.sy0 + 2)
                    add_prop("flower", tx + 3, wing.sy0 + 3)
                for tx in range(lab_x0 + 6, court_x1 - 4, 14):
                    add_prop("bench", tx, wing.sy1 - 2)
                    add_prop("hedge", tx + 5, wing.sy0 + SOUTH_H // 2)
                add_prop("bollard", lab_x0 + 2, wing.sy0 + SOUTH_H // 2)
                add_prop("bollard", court_x1 - 2, wing.sy0 + SOUTH_H // 2)
            ...
            if is_ground:
                ...
                for i, (ry0, ry1) in enumerate(east_rows):
                    ...
                    if i == 0:
                        # Wing library — three bookshelf bays along the outer wall
                        for bx in (inner_wall + 3, inner_wall + rw // 2, wing.ex1 - 2):
                            add_prop("bookshelf", bx, ry0 + 2)
                        add_prop("planter", inner_wall + 2, ry1 - 1)
                    targets.append((east_ids[i], (inner_wall + 1 + rw // 2, ry0 + rh // 2)))
                add_board(f"board-{key}-reception", wing.ex0, wing.ny0, 4)
                add_prop("planter", wing.ex0 + 1, wing.ny0 + 2)
                add_prop("planter", wing.ex0 + 1, wing.y0 + wing.h - 4)
                add_prop("bench", wing.ex0 + 2, wing.cmid - 3)
                add_prop("bench", wing.ex0 + 2, wing.cmid + 3)
    ...
    for wy in range(WALK[1] + 6, WALK[1] + WALK[3] - 6, 7):
        add_prop("bench", walk_mid - 5, wy)
        add_prop("tree", walk_mid + 6, wy + 2)
        add_prop("tree-sm", walk_mid - 7, wy + 3)
        add_prop("bush", walk_mid + 3, wy + 5)
        add_prop("flower", walk_mid - 2, wy + 1)
        add_prop("lamp", walk_mid, wy + 4)
        if wy % 14 < 7:
            add_prop("hedge", walk_mid + 8, wy)
            add_prop("planter", walk_mid - 8, wy + 2)

    # Street lamps flanking the main drive and the two cross drives
    for ty in range(12, gate_y - 2, 8):
        add_prop("lamp", DRIVE_X - 4, ty)
        add_prop("lamp", DRIVE_X + 4, ty)
        add_prop("bollard", DRIVE_X - 6, ty + 3)
        add_prop("bollard", DRIVE_X + 6, ty + 3)
    for tx in range(14, DRIVE_X - 4, 10):
        add_prop("lamp", tx, 29)
        add_prop("lamp", tx, 57)
        add_prop("hedge", tx, 27)
        add_prop("hedge", tx, 55)

    # Hedge line along the south face of each wing shell
    for bid, bx, by, bw, bh, door, storeys, label in exteriors:
        if bid == "notice-board":
            continue
        for hx in range(bx + 2, bx + bw - 2, 5):
            add_prop("hedge", hx, by + bh + 1)
            if hx % 10 == bx % 10:
                add_prop("flower", hx + 2, by + bh + 2)
        for hx in range(bx + 4, bx + bw - 4, 12):
            add_prop("planter", hx, by - 1)
        add_prop("bollard", bx + bw // 2 - 3, by + bh + 2)
        add_prop("bollard", bx + bw // 2 + 3, by + bh + 2)
