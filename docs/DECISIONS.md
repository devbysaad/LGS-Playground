...
## ADR-009 — 32×32 tiles, camera zoom 1.65

- **Date:** 2026-03 (updated 2026-08-09)
- **Decision:** Tile size is 32×32. Main camera zoom is **1.65** with `roundPixels: true` and light follow lerp.
- **Rationale:** Crisp pixel art at a readable scale for dense wing interiors; 1.5 left too much empty grass, 2.0 shimmered on some displays.
- **Revisit if:** High-DPI edge shimmer returns — fall back toward 1.5.
...
## ADR-024 — Excalidraw-derived two-wing campus layout

- **Date:** 2026-08-09
- **Decision:** The playable campus follows the hand-drawn Excalidraw site plan: two long two-storey academic wings (A-Level north, O-Level south), an east walking area, south parking + sports ground, and a main gate on the south wall. Library, Accounts/Admin, reception, Principal Office and canteen live in each wing's **east column**, not as free-standing buildings. `tools/gen_lgs_campus.py` stamps a `Wing` floor template; content ids are `a-level-block` / `o-level-block` with areas `a-level-ground`, `a-level-first`, `o-level-ground`, `o-level-first`.
- **Rationale:** Matches the user's drawn plan while keeping ADR-004 (one map, one scene, one room) and ADR-020/022 (interior islands + stair portals). Thirty classrooms stay in one `CLASS_PLAN` table so the count is trivial to retune.
- **Consequence:** Old ids (`block-a`, `admin-office`, `library`, `canteen`, `science-lab`, …) are retired. Orientation hunt contentVersion bumped to `3`. Facades and dense pixel props are procedural in `CampusDecor.ts`.
- **Revisit if:** An official Gudwal footprint arrives, or the east-column offices need to become separate enterable buildings again.
